...
package manager installed (e.g., via Anaconda). However, I recommend using `mamba`, via [miniforge](https://github.com/conda-forge/miniforge) as it is faster and more lightweight than `conda`.
...
```
conda create -n fred_charts python=3.12
conda activate fred_charts
```
...
The dependencies used in this environment (along with many other environments commonly used in data science) are stored in the conda environment called `fred_charts` which is saved in the file called `environment.yml`. To create the environment from the file (as a prerequisite to loading the environment), use the following command:
...
- Activate environment for this project: `conda activate fred_charts`
- Remove conda environment: `conda remove --name fred_charts --all`
